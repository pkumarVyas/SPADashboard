...
export default function NewSPAModal({ onClose }) {
  const [form, setForm]     = useState(INITIAL);
  const [status, setStatus] = useState(null); // null | 'loading' | result
  ...
  const setItem    = (f, v) => setForm(prev => ({ ...prev, ItemIdentifier: { ...prev.ItemIdentifier, [f]: v } }));
  const setPricing = (f, v) => setForm(prev => ({ ...prev, Pricing: { ...prev.Pricing, [f]: v } }));
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      Pricing: { ...form.Pricing, Price: parseFloat(form.Pricing.Price), MinQty: parseInt(form.Pricing.MinQty, 10) },
    };
    try {
            ...
            <div className={`result ${status.success ? 'success' : 'error'}`}>
              {status.success ? (
                <>
                  ...
                  <div>RecId: {status.data?.RecId} · Status: {status.data?.Status}</div>
                </>
              ) : status.queued ? (
                <>
